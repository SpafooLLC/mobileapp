...
namespace Netsam.Modules.MakeAppointment.Data
{
    ...
    public class SqlDataProvider : DataProvider
    {
        ...
        public override int AddProviderAppointment(int ClientID, int ProviderID, int AddressID, string ForDate, string AtTime,string EndTime,string PayTxnID, string CCNumber, string Expriry,string Comment,string PaymentProfileID,int EditAppID,decimal Discount)
        {
            return int.Parse(SqlHelper.ExecuteScalar(ConnectionString, NamePrefix + "AddProviderAppointment", ClientID, ProviderID, AddressID, ForDate, AtTime, EndTime, PayTxnID, CCNumber, Expriry, Comment, PaymentProfileID, EditAppID,Discount).ToString());
        }
        ...
        {
            SqlHelper.ExecuteNonQuery(ConnectionString, NamePrefix + "UpdateSeenStatus", ID);
        }
        ...
        {
            SqlHelper.ExecuteNonQuery(ConnectionString, NamePrefix + "HideApp4Me", AppID, UserType);
        }

        public override bool IsProviderSlotFree(int ProID, string StartDateTime, string EndDateTime)
        {
            int rValue = int.Parse(SqlHelper.ExecuteScalar(ConnectionString, NamePrefix + "IsFreeSlotFor", ProID, StartDateTime, EndDateTime).ToString());
            if (rValue == 1)
                return true;
            else
                return false;
        ...
        {
            int rValue = int.Parse(SqlHelper.ExecuteScalar(ConnectionString, NamePrefix + "IsFreeSlotForEM", ProID, StartDateTime, EndDateTime, AppID).ToString());
            if (rValue == 1)
                return true;
            else
                return false;
        ...
        public override System.Data.IDataReader GetProOccupiedSlots(int ProviderID, string FromDate,string ToDate)
        {
            return SqlHelper.ExecuteReader(ConnectionString, NamePrefix + "GetProOccupiedSlots", ProviderID, FromDate,ToDate);
        }

        public override bool CanSetAvailability(int ProID, string StartDateTime, string EndDateTime)
        {
            int rValue = int.Parse(SqlHelper.ExecuteScalar(ConnectionString, NamePrefix + "CanSetAvailability", ProID, StartDateTime, EndDateTime).ToString());
            if (rValue == 1)
                return true;
            else
                return false;
        ...
        //public override void RemoveAvailSlot(int ProID, string StartDateTime, string EndDateTime)
        //{
        //    SqlHelper.ExecuteNonQuery(ConnectionString, NamePrefix + "RemoveAvailSlot", ProID, StartDateTime, EndDateTime);
        //}

        public override System.Data.IDataReader GetUsersInRole(string RoleName,int CP,int RPP)
        {
            return SqlHelper.ExecuteReader(ConnectionString, NamePrefix + "GetUsersInRole", RoleName, CP, RPP);
        }
        #endregion
...
#region Rating Method
        ...
        {
            return int.Parse(SqlHelper.ExecuteScalar(ConnectionString, NamePrefix + "AddRating", RatingByID, RatingToID, RatingValue, RatingTypeID,AppID).ToString());
        }
        ...
        {
            return int.Parse(SqlHelper.ExecuteScalar(ConnectionString, NamePrefix + "AddUserReview", ByUserID, ToUserID, ILike, IDLike, Comments, DisplayNameAs,AppID).ToString());
        }
        ...
        {
            string _rValue = SqlHelper.ExecuteScalar(ConnectionString, NamePrefix + "DidIRated", ByUserId, AppID).ToString();
            if (_rValue == "0") { return false; }
            else { return true; }
        }
#endregion
        ...
        public override System.Data.IDataReader GetAdminNotifications(int Page, int RecsPerPage)
        {
            return SqlHelper.ExecuteReader(ConnectionString, NamePrefix + "GetAdminNotifications", Page, RecsPerPage);
        }
