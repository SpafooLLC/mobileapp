...
function NSD_MA_ShowCalendar(e) {
    ...
    $('#NS_dvCaldendar').fullCalendar({
        ...
        select: function (start, end, a, b, c) {
            ...
            var _m = moment();
            var _IsToday = (_m.format("DD-MM-YYYY") == start._i.format('dd-MM-yyyy'));
            var SliderStartHours = 9;
            var SliderStartMinutes = 0;
            var SliderEndHours = 21;
            ...
            var _Title = NS_FormatDate(Date.parse(start._i), 'mm/dd/yy');
            $("#lblSlotDate").text(_Title);
            $("#lblServiceSession").text(NS_Session + ' minute(s)');
            var defaultEnd = new Date(start._i.getFullYear(), start._i.getMonth(), start._i.getDate(), finalH, tempM);
            NS_MA_IsSlotAvailable = false;
            $("#imgAvail").hide();
            $("#lblAvail").hide();
            var dialog = $("#NS_dvSliderOuter").dialog({
                autoOpen: true, height: 450, width: 900, modal: true,
                title: 'Choose you time slot',
                buttons: {
                    "Ok": SaveTimeSlot,
                    Cancel: function () {
                        dialog.dialog("close");
...
function NSD_MA_ShowCreditCard(e) {
    ...
    if (NSD_MA_SelectedProfileID != "0" && NSD_MA_SelectedProfileID != undefined) {
        bootbox.confirm('<div class="ccharge"> <p><i class="fa fa-info-circle fa-3x"></i> </p>' + 'Your card ending in <strong>' + rblSelectedText + '</strong> will be charged for amount of <span class="camo">$' + oOther.other[0].OrderTotal + ' USD</span><br/>You will be charged $25 for a cancellation within 12 hours of the start of your requested appointment time  or if ASAP appointment which is included in the service fee' + '</div>',
            function (r) {
                if (r) {
...
function NS_AddAddress_SuCB(d) {
    ...
    var _data = '';
    if (!NS_MA_IsSoonest)
        _data = "{'ClientID':'" + NS_MA_ClientID + "','ProviderID':'" + NSD_MA_CurrentPro.split(":")[0] + "','AddressID':'" + d + "','ForDate':'" + oTimeSlot.TimeSlot[0].dated + "','AtTime':'" + oTimeSlot.TimeSlot[0].start + "','EndTime':'" + oTimeSlot.TimeSlot[0].end + "','CSVSRVC':'" + CSVService + "','PayTxnID':'" + oUserInput.PayTxnID + "','CCNumber':'" + oUserInput.CardNumber + "','Expriry':'" + oUserInput.Expiry + "','Comment':'" + _Comment + "','oAuthTxn':'" + oUserInput.oAuthTxnID + "','EditAppID':'" + EditAppID + "'}";
    else {
        _data = "{'ClientID':'" + NS_MA_ClientID + "','ProviderID':'" + NSD_MA_CurrentPro.split(":")[0] + "','AddressID':'" + d + "','ForDate':'','AtTime':'','EndTime':'','CSVSRVC':'" + CSVService + "','PayTxnID':'" + oUserInput.PayTxnID + "','CCNumber':'" + oUserInput.CardNumber + "','Expriry':'" + oUserInput.Expiry + "','Comment':'" + _Comment + "','oAuthTxn':'" + oUserInput.oAuthTxnID + "','EditAppID':'" + EditAppID + "'}";
    }
...
function NS_VerifyCardInput() {
    if (NSD_MA_SelectedProfileID == "0" || NSD_MA_SelectedProfileID == undefined) {
        ...
        if (!_CCExpression.test(oUserInput.CardNumber)) {
            bootbox.alert('Credi card seems to be invalid, please specify a valid card number'); return false;
        }
...
function CreateCCProfile() {
    ...
    NS_MakeRequest(_URL, _data, function (d) {
        ...
        if (d.profile != null) {
            ...
            var _data = "{'PID':'" + NSD_MA_CPayProfile.profile.customerProfileId + "','CCNumber':'" + CCNumber + "','Expiry':'" + Expiry + "','CVV':'" + CVV + "'}";
            ...
            if (NSD_MA_CPayProfile == '') {
                ...
                var _data = "{'UID':'" + NS_MA_LoggedUser.UserID + "','CCNumber':'" + CCNumber + "','Expiry':'" + Expiry + "','CVV':'" + CVV + "','Email':'" + NS_MA_LoggedUser.Email + "'}";
                NS_MakeRequest(_URL, _data, function (d) {
                    NS_MA_SaveAppointment();
                });
            }
