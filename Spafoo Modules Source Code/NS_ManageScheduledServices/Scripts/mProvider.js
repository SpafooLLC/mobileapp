...
function CaptureCardOrProfile(o) {
    if ($(o).text() == 'Appointment Completed') {
        var Amount = (oAppointment.Amount - oAppointment.Discount);
        if (Amount > 0) { // charge profile only when the amount is greater than ZERO
            $(o).text("Please wait...");
            //  ChargePreviousAuth
            var _URL = "/DesktopModules/NS_MakeAppointment/rh.asmx/ChargeProfile"; //ChargePreviousAuth
            var _data = "{'PID':'" + oAppointment.CustomerProfileID + "','PPID':'" + oAppointment.PayProfileID + "','amount':'" + Amount + "'}";
        ...
        else { // if total amount to pay is nothing after discount then , do go for payment process but just mark the appointment as done
            NS_AddNotification(oAppointment.ProviderID, oAppointment.ClientID, 8, oAppointment.AppointmentID, function () {
                oAppointment.PayTxnID = "-99";
                UpdateAppointment();
            })
        }
    }
