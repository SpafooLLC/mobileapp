...
namespace Netsam.Modules.MakeAppointment.Components
{
    ...
    public class AppointmentController //: IPortable, ISearchable, IUpgradeable
    {
        ...
        {
            return DotNetNuke.Common.Utilities.CBO.FillObject<NS_UserInfo>(SqlDataProvider.Instance().GetUserBasicInfo(UserID));
        }
        ...
        public int AddAppointment(int ClientID, int ProviderID, int AddressID, string ForDate, string AtTime,string EndTime,string PayTxnID, string CCNumber, string Expriry,string Comment,string PayProfileID,int EditAppID,decimal Discount,string AnyProviderIds) {
            return SqlDataProvider.Instance().AddProviderAppointment(ClientID, ProviderID, AddressID, ForDate, AtTime, EndTime, PayTxnID, CCNumber, Expriry, Comment, PayProfileID, EditAppID,Discount, AnyProviderIds);
        }
        ...
        public List<AppointmentInfo> ListAllAppointmentByAny()
        {
            //try
            //{
                return DotNetNuke.Common.Utilities.CBO.FillCollection<AppointmentInfo>(DataProvider.Instance().ListAllAppointmentByAny());
            //}
            //catch (Exception ex)
            //{
            //    List<AppointmentInfo> lst = new List<AppointmentInfo>();
            //    AppointmentInfo oApp = new AppointmentInfo();
            //    oApp.Comments = ex.InnerException.ToString();
            //    lst.Add(oApp);
            //    return lst;
            //}
        }
        ...
        {
            return DotNetNuke.Common.Utilities.CBO.FillCollection<BasicAppointmentInfo>(DataProvider.Instance().ListAllAppointmentByAny());
    ...
    public class BasicAppointmentInfo
    {
        ...
        public NS_UserInfo ClientInfo
        {
            get
            {
                ...
                {
                    AppointmentController oCtrl = new AppointmentController();
                    NS_UserInfo tUser = oCtrl.GetBasicUserInfo(this.ClientID);
                    return tUser;
                }
                catch (Exception ex)
                {
        ...
        public NS_UserInfo ProviderInfo
        {
            get
            {
                ...
                {
                    AppointmentController oCtrl = new AppointmentController();
                    NS_UserInfo tUser = oCtrl.GetBasicUserInfo(this.ProviderID);
                    return tUser;
                }
                catch (Exception ex)
                {
