    ...
    public class NotificationController //: IPortable, ISearchable, IUpgradeable
    {
        ...
        private string GetUserMessage(int NotificationTypeID, int RelatedEntityID)
        {
            ...
            try
            {
                ...
                if (oApp != null)
                {
                    ...
                    string ProviderName = "";
                    if (oApp.ProviderID != -1)
                    {
                        ProviderName = oApp.ProviderInfo.FirstName + " " + oApp.ProviderInfo.LastName.Substring(0, 1) + ".";
                    }
                    rValue = "2";
        ...
        public string SendNotification(int UserID, int RoleID, string Message)
        {
            ...
            try
            {
                ...
                if (UserID != -1)
                {
                    rValue= SendNotificationToUser(UserID, Message);
                    if (rValue == "0") { rValue= "Device not found, cannot send message"; }
                    else
                    {
                        rValue = "Message send successfully";
                    }
                }

                // SEND TO ALL USERS IN GIVEN ROLE
                if (RoleID != -1)
                {
                    RoleController oRctrl = new RoleController();
                    // get role detail by RoleID
                    RoleInfo oRole = oRctrl.GetRoleById(0, RoleID);
                    if (oRole != null)
                    {
                        // Get all the users in the given role
                        List<UserInfo> lstUsers = new List<UserInfo>(oRctrl.GetUsersByRole(0, oRole.RoleName));
                        // Loop through each users and send notification message to each users
                        foreach (UserInfo oUser in lstUsers)
                        {
                            if (oUser != null)
                            {
                                rValue = SendNotificationToUser(oUser.UserID, Message);
                            }
                        }
                        if (lstUsers.Count > 0)
                            rValue = "Message send successfully.";
                        else
                            rValue = "User(s) not found in this role, cannot send message.";
                    }
                }
            }
            catch (Exception ex)
        ...
        private string SendNotificationToUser(int UserID, string Message)
        {
            ...
            if (UserID != -1)
            {// send to single user
                List<NS_UserHardware> lstUHInfo = oUHCtrl.GetUserHardware(UserID);
                foreach (NS_UserHardware oUHInfo in lstUHInfo)
                {
                    if (oUHInfo != null)
                    {
                        if (!oUHInfo.IsWebVersion)
                        {
                            ...
                            if (oUHInfo.DeviceToken.Trim() != "")
                            {
                                if (Message.Trim() != "")
                                {
                                    oCtrl.SendAndroidNotification(oUHInfo.DeviceToken, Message);
                                    oCtrl.SendToiOS(oUHInfo.DeviceToken, Message);
                                    rValue = "1";
                                }
                            }
                        }
