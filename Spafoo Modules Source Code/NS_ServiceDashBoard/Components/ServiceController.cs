...
namespace Netsam.Modules.ServiceDashBoard.Components
{
    ...
    public class ServiceController //: IPortable, ISearchable, IUpgradeable
    {
        ...
        public List<ServiceInfo> GetServicesIn2(int ServiceID,int ProviderID)
        {
            return DotNetNuke.Common.Utilities.CBO.FillCollection<ServiceInfo>(DataProvider.Instance().GetServicesIn2(ServiceID, ProviderID));
        }
        ...
        {
            return DataProvider.Instance().AddService(oService.ServiceID, oService.ServiceName, oService.ShortDescription, oService.Image, oService.ParentID, oService.Price, oService.Duration);
        }
        ...
        public List<UserInfo> ListProviders(string SIDs) {
            List<UserInfo> lstUsers = new List<UserInfo>();
            SqlDataReader  dr =(SqlDataReader ) DataProvider.Instance().ListProvidersByService(SIDs);
        ...
        public void AssignToFormUser(string UserName, int ServiceID)
        {
            DataProvider.Instance().AddFormUserService(UserName, ServiceID);
        }

        public void RemoveFormUserService(string Username)
        {
            DataProvider.Instance().RemoveFormUserService(Username);
        }

        public List<ServiceInfo> GetFormUserServices(string Username)
        {
            return DotNetNuke.Common.Utilities.CBO.FillCollection<ServiceInfo>(DataProvider.Instance().GetFormUserServices(Username));
        }
    ...
    public class UserRatingController
    {
        ...
        public UserReviewInfo GetRaReOnApp(int UserID, int AppID)
        {
            return DotNetNuke.Common.Utilities.CBO.FillObject<UserReviewInfo>(DataProvider.Instance().GetMyRaReOnApp(UserID, AppID));
        }
    }
    ...
    public class CouponsController
    {
        ...
        {
            return DotNetNuke.Common.Utilities.CBO.FillObject<CouponInfo>(DataProvider.Instance().GetCoupon(ID));
        }
        
        public CouponInfo ValidateCoupon(string Code)
        {
            return DotNetNuke.Common.Utilities.CBO.FillObject<CouponInfo>(DataProvider.Instance().ValidateCoupon(Code));
        }
        
        public void RemoveCoupon(int ID)
        {
            DataProvider.Instance().RemoveCoupon(ID);
        }
        ...
        {
            return int.Parse(DataProvider.Instance().AddCoupon(Name, Code, Discount, DiscountType, StartsFrom, EndsOn, NoOfCoupons).ToString());
        }
        
        public void UpdateCoupon(int ID, string Name, string Code, int Discount, string DiscountType, DateTime StartsFrom, DateTime EndsOn, int NoOfCoupons)
        {
            DataProvider.Instance().UpdateCoupon(ID, Name, Code, Discount, DiscountType, StartsFrom, EndsOn, NoOfCoupons);
        }
        
        public List<CouponInfo> ListCoupon()
        {
            return DotNetNuke.Common.Utilities.CBO.FillCollection<CouponInfo>(DataProvider.Instance().ListCoupon());
        }
        
        public void UpdateCouponCount(string Code)
        {
            DataProvider.Instance().UpdateCouponCount(Code);
        }
    }
