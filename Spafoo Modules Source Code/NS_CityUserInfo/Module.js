...
function NS_City_ValidateForm() {
    ...
    if ($("#NS_CityEmailYes").is(":checked")) {
        ...
        if ($.trim($("#NS_City_ConfirmEmail").val()) == '') {
            alert('Please provide information Confirm E-mail Address');
            $("#NS_City_ConfirmEmail").focus();
            return false;
        }
        else {
            if (!NS_City_IsValidEmail($.trim($("#NS_City_ConfirmEmail").val()))) {
                alert('Confirm E-mail Address found to be in invalid format');
                $("#NS_City_ConfirmEmail").focus();
                return false;
            }
        }
        // E-mail Address Match
        if ($.trim($("#NS_City_ConfirmEmail").val()) != $.trim($("#NS_City_Email").val())) {
            alert('Your E-mail address do not match. Please try again');
            $("#NS_City_Email").focus();
            return false;
        }
    }
    ...
    // SSN Document
    //if (NS_City_SSNDocument==""){
    //    alert("Please upload related document for Social Security");
    //    return false;
    //}
    // State Document
    if ($("#NS_CityTypeOfIDState").is(":checked")) {
        if (NS_City_StateIDDocument == "") {
    ...
    if ($("#NS_CityTypeOfIDLicense").is(":checked")) {
        if (NS_City_LicenseDocument == "") {
            alert("Please upload related document for Drivers License");
            return false;
        }
    }
    if ($("#NS_City_BankDraftYes").is(":checked")) {
        if (NS_City_CancelCheckDocument == "") {
            alert("Please upload related document for Cancelled Cheque");
            return false;
        }
    }
    // Proof of Address Document
    if (NS_City_POADocument=="")
    {
...
function NS_City_SaveForm(AsEmail) {
    ...
    $.ajax({
        ...
        success: function(r) {
            if (AsEmail == "Y") {
                $("#NS_City_dvCofirmationMessage").dialog({ width: 500,modal: true,
                     buttons: {
...
function NS_City_OpenDetail(ID) {
    $("#dvViewDetails_" + ID).dialog({ width: 500, height: 500, modal: true,
        buttons: {
