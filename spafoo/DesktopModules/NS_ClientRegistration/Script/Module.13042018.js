...
$(document).ready(function () {
    ...
    $('[id*="btnShowEmailManager"]').click(function () {
        if ($(this).text() == "Back") {
            $('[id*="pnlEmailContent"]').hide();
            $('#dvEmailContent').hide();
            $("#dvClientStep1").show();
            $(".btnend").show();
            $(this).text("Manage e-mail content");
            $("#lblHeader").text("Basic Profile Information");
        }
        else {
            $('[id*="pnlEmailContent"]').show();
            $('#dvEmailContent').show();
            $("#dvClientStep1").hide();
            $(".btnend").hide();
            $(this).text("Back");
            $("#lblHeader").text("Update Email Content");
        }
        return false;
    });
});
...
function NSR_SaveUserApplication(IsLater) {
    ...
    //UN, FN, LN, EM, P, PID
    var data = "{'UN':'" + UN + "','FN':'" + FN + "','LN':'" + LN + "','EM':'" + EM + "','P':'" + P + "','PID':'" + NSR_PID + "','S':'" + S + "','C':'" + C + "','R':'" + R + "','Z':'" + Z + "','Ph':'" + Ph + "','Mo':'" + Mo + "','PicFID':'" + oPicid + "','HN':'','DT':'','IsWeb':'1','GDR':'" + Gender + "','MID':'" + NSR_MID + "'}";
