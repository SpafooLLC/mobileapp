...
function NS_GetProRating(ID,R, P) {
    if (R > 0) {
        $("#NSD_starRating_" + ID).barrating('set', R)
        $("#NSD_starRating_" + ID).barrating('readonly', true);
    }
}
function NS_GetIDRating(ID,R, P) {
    if (R > 0) {
        $("#NSD_starRatingID_" + ID).barrating('set', R)
        $("#NSD_starRatingID_" + ID).barrating('readonly', true);
        $("#NSD_lblRatingID_" + ID).text("from " + P + " users");
    }
    else {
        $("#NSD_starRatingID_" + ID).barrating('set', 0);
        $("#NSD_lblRatingID_" + ID).text("");
    }
}